...
// dynamic imports removed for small client components to avoid runtime chunk load errors
...
export default function HomePage(): JSX.Element {
  ...
  const [activeTools, setActiveTools] = useState<{
    ...
    sticky?: boolean;
    edit?: boolean;
  }>({ add: false, replace: false, ai: false, sticky: false, edit: false });
        ...
        try {
          ...
          if (Array.isArray(parsed)) {
            if (parsed.length === 0) {
              ...
              const notes = parsed.map((s: string, i: number) => ({
              ...
              setHighestZIndex(
                notes.reduce((m, n) => Math.max(m, n.zIndex || 0), 1000),
              );
            } else {
              // Ensure any persisted notes have runtime fields
              const notes = parsed.map((n: any, i: number) => ({
                x: n.x ?? 60 + i * 24,
                y: n.y ?? 80 + i * 24,
                zIndex: typeof n.zIndex === "number" ? n.zIndex : 1000 + i,
                content: n.content ?? "",
  ...
  const handleToolToggle = (tool: string) => {
    setActiveTools((prev) => {
      switch (tool) {
        case "replace":
          return { ...prev, replace: !prev.replace };
        case "add":
          return { ...prev, add: !prev.add };
        case "edit":
          return { ...prev, edit: !prev.edit };
        case "aiContent":
          return { ...prev, ai: !prev.ai };
        case "stickyNotes":
  ...
  const handleOpenNote = (id: string | number) => {
    ...
    setStickyNotes((prev) => {
      ...
      const updated = prev.map((n) =>
        n.id === id ? { ...n, visible: true, zIndex: nextZ } : n,
      );
  ...
  const handleBringToFront = (id: string | number) => {
    setHighestZIndex((prev) => {
      const next = prev + 1;
      setStickyNotes((notes) =>
        notes.map((n) => (n.id === id ? { ...n, zIndex: next } : n)),
      );
      return next;
    });
  };
  ...
  const handleRenameNote = (id: string | number, newTitle: string) => {
    setStickyNotes((prev) =>
      prev.map((n) => {
        if (n.id !== id) return n;
        const prevHistory = n.titleHistory
          ? n.titleHistory.slice(0, (n.titleHistoryIndex ?? 0) + 1)
          : [n.title];
  ...
  const handleApplyAdd = () => {
    ...
    if (!current)
      return flashStatus(
        "No content to apply Add operation to.",
        2000,
        "error",
      );
    ...
    flashStatus(
      "Inserted configured snippets into the document.",
      2000,
      "success",
    );
  };
  ...
  const handleApplyReplace = () => {
    ...
    if (!current)
      return flashStatus(
        "No content to apply Replace operation to.",
        2000,
        "error",
      );
  ...
  const handleFormat = (cmd: "bold" | "italic" | "underline") => {
    try {
      ...
      import("@/lib/logger")
        .then(({ error: logError }) => logError(e))
        .catch(() => {});
  ...
  const handleToggleAddEnabled = (val: boolean) => {
    ...
    flashStatus(
      val ? "Add pipeline enabled" : "Add pipeline disabled",
      1600,
      "info",
    );
  };
  ...
  const handleToggleReplaceEnabled = (val: boolean) => {
    ...
    // Do not auto-close panels when toggling pipeline enable; user controls selection explicitly
    flashStatus(
      val ? "Replace pipeline enabled" : "Replace pipeline disabled",
      1600,
      "info",
    );
  };
  ...
  const handleAddSettingsPatch = (patch: Partial<AddSettings>) => {
    setAddSettings((prev) => {
      ...
      merged.contents = (merged.contents || prev.contents || []).slice(
        0,
        merged.numFields,
      );
      while (merged.contents.length < merged.numFields)
        merged.contents.push("");

      // Normalize baseValues according to mode
      if (merged.mode === "alternative" || merged.numFields === 1) {
        merged.baseValues = [
          merged.baseValues?.[0] ?? prev.baseValues?.[0] ?? 16,
        ];
      } else if (merged.mode === "sequential") {
        merged.baseValues = (merged.baseValues || prev.baseValues || []).slice(
          0,
          merged.numFields,
        );
        while (merged.baseValues.length < merged.numFields)
  ...
  const handleReplaceSettingsPatch = (patch: Partial<ReplaceSettings>) => {
    setReplaceSettings((prev) => {
      ...
      let replaces = (merged.replaces ?? prev.replaces ?? []).slice(
        0,
        numReplace,
      );
  ...
  const handleFileChange: React.ChangeEventHandler<HTMLInputElement> = async (
    ...
    try {
      if (ext === "txt" || file.type === "text/plain") {
        ...
        flashStatus(
          "Unsupported import type (PDF/DOCX not supported).",
          3200,
          "error",
        );
      }
  ...
  const handleProcessData = () => {
    ...
    try {
      ...
      if (isAddEnabled)
        processedContent = applyAddOperation(processedContent, addSettings);
      if (isReplaceEnabled)
        processedContent = applyReplaceOperation(
          processedContent,
          replaceSettings,
        );
      ...
      const titleHtml =
        title && title.trim()
          ? `<h2 style="font-size: 1.25rem; font-weight: bold; margin-bottom: 1rem;">${escapeHtml(title)}</h2>`
          : "";
      ...
      import("@/lib/logger")
        .then(({ error: logError }) => logError(err))
        .catch(() => {});
  ...
  const handleClearAll = () => {
    ...
    flashStatus(
      "Cleared editor, settings, and sidebar selections.",
      1500,
      "info",
    );
  };
  ...
  const handleDownloadPDF = () => {
    ...
    (async () => {
      try {
        ...
        const fileName =
          (title || "document").replace(/[^a-z0-9_\-]/gi, "_") + ".pdf";
        doc.save(fileName);
        flashStatus("PDF downloaded.", 1800, "success");
      } catch (err) {
        try {
          const { error: logError } = await import("@/lib/logger");
          logError("Failed to load jspdf", err);
        } catch {}
      ...
      <div className="space-y-3">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 bg-black/40 border border-white/10 rounded px-2 py-2 text-xs"
          />
          <button
            onClick={() => {
              if (val.trim()) {
            ...
            className="py-2 px-3 bg-white/5 rounded"
          >
            Add
          </button>
        </div>
        <div className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar">
          ...
          {notes.map((n, i) => (
            <div
              key={i}
              className="p-2 bg-white/5 rounded flex items-start justify-between"
            >
              <div className="text-xs">{n}</div>
              <button onClick={() => onRemove(i)} className="ml-2 text-red-400">
                ×
              </button>
            </div>
          ))}
              ...
              <div className="home-ai-section">
                <AIContentGenerator
                  onGenerated={(generatedTitle, generatedContent) => {
                    ...
                    flashStatus(
                      "AI content generated and inserted",
                      1800,
                      "success",
                    );
                  }}
          ...
          <div className="home-output-section">
            ...
            <div
              ...
              dangerouslySetInnerHTML={{
                __html: cleanOutputHtml(processedOutputHtml),
              }}
