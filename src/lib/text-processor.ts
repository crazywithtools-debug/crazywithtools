...
export function applyAddOperation(
  ...
  const processWithWalker = (
    root: HTMLElement,
    currentBaseValue: number,
    currentNewContents: string[],
  ) => {
    if (currentBaseValue <= 0) return;
    const SHOW_TEXT =
      typeof (globalThis as any).NodeFilter !== "undefined"
    ...
    while (node) {
      ...
      try {
        if (
          Array.isArray(excludeHeadingTags) &&
          excludeHeadingTags.length > 0
        ) {
          const parentEl = (textNode as any)
          ...
          if (parentEl) {
            ...
            for (const tag of excludeHeadingTags) {
              ...
              try {
                if (
                  (parentEl as HTMLElement).closest &&
                  (parentEl as HTMLElement).closest(tag)
                ) {
                  skipThisNode = true;
                  break;
                }
...
export function applyReplaceOperation(
  ...
  for (let i = 0; i < finds.length; i++) {
    ...
    const replaceWith =
      replaces[i] !== undefined ? replaces[i] : replaces[0] || "";
    ...
    try {
      ...
      import("./logger")
        .then(({ error: logError }) => logError(`Invalid regex: ${find}`, e))
        .catch(() => {});
    }
  }
