...
const AIContentGenerator: FC<AIContentGeneratorProps> = ({
  apiKey,
  onGenerated,
  onClose,
}) => {
  ...
  const LENGTH_INSTRUCTION =
    "Generate content of any length according to the prompt requirements.";
  const CONTENT_STRUCTURE_INSTRUCTIONS =
    'Return content as clean HTML using <p>, <ul>, <li>, <strong>, <h2>, <h3> tags. Structure with appropriate paragraphs, bullet lists if relevant, and a closing call-to-action. Return ONLY valid JSON with "title" and "content" keys.';
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const res = await generateContent({ prompt: fullPrompt, apiKey }, 170_000);
      if (!res.ok) {
        setError(res.friendly || res.error || "Generation failed");
        logError("AI Generation Error:", res.error || res);
        return;
      }

      const body = res.data as GenerateResponseBody;
  ...
  return (
    <div
      className="flex flex-col bg-white/5 overflow-hidden transition-all duration-200"
    ...
    >
      <div className="p-2 max-w-4xl mx-auto w-full flex flex-col overflow-hidden">
        ...
        <div className="shrink-0 mt-2">
          {error && (
            <p className="text-xs text-red-500 font-medium transition-opacity duration-200">
              ✗ {error}
            </p>
          )}
        </div>
