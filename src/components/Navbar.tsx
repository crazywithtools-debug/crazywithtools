...
function Navbar(): JSX.Element {
  ...
  useEffect(() => {
    // HMR probe: logs when Navbar re-renders during Fast Refresh
    if (typeof window !== "undefined") console.debug("Navbar HMR update");
  }, []);
    ...
    try {
      ...
      if (saved) {
        ...
        root.style.setProperty(
          "--active-color",
          parsed.activeColor || "#ffffff",
        );
        root.style.setProperty(
          "--button-bg-color",
          parsed.buttonBgColor || "#10B981",
        );
        root.style.setProperty(
          "--button-text-color",
          parsed.buttonTextColor || "#000000",
        );
        root.style.setProperty("--theme-color", parsed.themeColor || "#ffffff");
        if (parsed.backgroundImage) {
          root.style.setProperty(
            "--bg-image",
            `url(${parsed.backgroundImage})`,
          );
      ...
      <div className="w-full lg:w-3/5 mx-auto">
        <div className="w-full h-[45px] md:h-[50px] flex items-center justify-between gap-3 md:gap-4">
          <div className="flex-1 h-full flex items-center justify-end">
            <div
              className="w-full h-full border rounded-full px-3 md:px-6 flex items-center justify-between shadow-2xl"
              style={{
            ...
            >
              <Link href="/" className="flex items-center gap-2 group shrink-0">
                <span
                  className="font-black text-lg md:text-xl uppercase tracking-tighter"
